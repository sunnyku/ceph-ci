  ...
  std::shared_ptr<CommandContext>& cmdctx)
{
  ...
  if (prefix == "pg scrub" ||
    ...
    for (auto osd : osds) {
      vector<spg_t> spgs;
      epoch_t epoch;
      cluster_state.with_pgmap_and_osdmap([&](const PGMap& pgmap, const OSDMap& osdmap) {
	  epoch = osdmap.get_epoch();
    ...
    r = cluster_state.with_pgmap_and_osdmap([&](const PGMap& pgmap, const OSDMap& osdmap) {
    ...
    bool one_pool = false;
    r = cluster_state.with_pgmap_and_osdmap([&](const PGMap& pg_map, const OSDMap& osdmap) {
        if (!pool_name.empty()) {
    ...
    int affected_pgs = 0;
    cluster_state.with_pgmap_and_osdmap([&](const PGMap& pg_map, const OSDMap& osdmap) {
