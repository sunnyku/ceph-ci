...
int ceph_error_category::from_code(int ev) const noexcept {
  ...
  case ceph::errc::does_not_exist:
    // What we use now.
    return -ENOENT;
  case ceph::errc::failure:
    return -EIO;
  case ceph::errc::exists:
    return -EEXIST;
  case ceph::errc::limit_exceeded:
    return -EIO;
  case ceph::errc::auth:
    return -EACCES;
  }
