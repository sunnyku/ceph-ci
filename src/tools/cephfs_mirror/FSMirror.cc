...
public:
  MirrorAdminSocketHook(CephContext *cct, std::string_view fs_name, FSMirror *fs_mirror)
    ...
    cmd = "cephfs mirror status " + std::string(fs_name);
...
void FSMirror::run() {
  ...
  while (true) {
    ...
    dout(20) << ": trying pick from " << m_directories.size() << " directories" << dendl;
    m_cond.wait(locker, [this]{return m_directories.size();});
  }
}
...
void FSMirror::status(Formatter *f) {
  std::scoped_lock locker(m_lock);
