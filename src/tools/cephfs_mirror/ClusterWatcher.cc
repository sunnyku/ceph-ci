...
void ClusterWatcher::handle_fsmap(const cref_t<MFSMap> &m) {
  ...
  std::list<std::string> mirroring_enabled;
  std::list<std::string> mirroring_disabled;
  ...
  std::map<std::string, uint64_t> fs_metadata_pools;
  {
    ...
    for (auto &filesystem : filesystems) {
      ...
      if (!mirror_info.is_mirrored()) {
        ...
        auto p = m_filesystem_peers.emplace(fs_name, Peers{});
        auto enabled = p.second;
        auto &peers = p.first->second;
