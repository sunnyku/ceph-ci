  ...
  const hobject_t &soid)
{
  ...
  if (p != recovering.end()) {
    ...
    if (is_missing_object(soid)) {
      recover_missing(soid, v, cct->_conf->osd_force_recovery_op_priority, h);
    ...
    pgbackend->run_recovery_op(h, cct->_conf->osd_force_recovery_op_priority);
  }
}
  ...
  bool *work_started)
{
  ...
  if (soid.snap && soid.snap < CEPH_NOSNAP) {
	...
	int r = recover_missing(
	    head, pg_log.get_missing().get_items().find(head)->second.need,
