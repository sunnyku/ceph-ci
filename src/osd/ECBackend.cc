...
struct SendPushReplies : public Context {
  ...
  void finish(int) override {
    std::vector<std::pair<int, Message*>> messages(replies.size());
...
void ECBackend::do_read_op(ReadOp &op)
{
  ...
  std::vector<std::pair<int, Message*>> m(messages.size());
...
bool ECBackend::try_reads_to_commit()
{
  ...
  std::vector<std::pair<int, Message*>> messages(get_parent()->get_acting_recovery_backfill_shards().size());
