// -*- mode:C++; tab-width:8; c-basic-offset:2; indent-tabs-mode:t -*-
// vim: ts=8 sw=2 smarttab
/*
 * Ceph - scalable distributed file system
 *
 * Copyright (C) 2019 Red Hat <contact@redhat.com>
 * Author: Adam C. Emerson <aemerson@redhat.com>
 *
 * This is free software; you can redistribute it and/or
 * modify it under the terms of the GNU Lesser General Public
 * License version 2.1, as published by the Free Software
 * Foundation.  See file COPYING.
 *
 */

#pragma once

#include <boost/system/error_code.hpp>

#include "include/rados.h"

const boost::system::error_category& osd_category() noexcept;

// Since the OSD mostly uses POSIX error codes plus a couple
// additions, this will be a degenerate error category for now that
// mostly forwards to POSIX.

namespace osd_errc {
enum osd_errc_t {
  old_snapc = 85,  /* ORDERSNAP flag set; writer has old snapc*/
  blacklisted = 108 /* blacklisted */
};
}

namespace boost {
namespace system {
template<>
struct is_error_code_enum<::osd_errc::osd_errc_t> {
  static const bool value = true;
};
}
}

namespace osd_errc {
//  explicit conversion:
inline boost::system::error_code make_error_code(osd_errc_t e) noexcept {
  return { e, osd_category() };
}

// implicit conversion:
inline boost::system::error_condition make_error_condition(osd_errc_t e)
  noexcept {
  return { e, osd_category() };
}
}
