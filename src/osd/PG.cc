...
bool PG::can_discard_op(OpRequestRef& op)
{
  ...
  if (m->get_connection()->has_feature(CEPH_FEATURE_RESEND_ON_SPLIT)) {
    // >= luminous client
    if (m->get_connection()->has_feature(CEPH_FEATURE_SERVER_MIMIC)) {
      // >= mimic client
	...
	return true;
      ...
      // == luminous client
      if (m->get_map_epoch() < pool.info.get_last_force_op_resend_luminous()) {
	dout(7) << __func__ << " sent before last_force_op_resend_luminous "
		<< pool.info.last_force_op_resend_luminous
		<< ", dropping" << *m << dendl;
	return true;
      }
    }
