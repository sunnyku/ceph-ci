  ...
  OpRequestRef orig_op)
{
  ...
  op.set_tolerated_uncommit_size(get_osdmap(), get_info().pgid.pgid);
...
void ReplicatedBackend::op_commit(const ceph::ref_t<InProgressOp>& op)
{
  ...
  if (op->waiting_for_commit.empty()) {
    ...
    op->on_commit->quorum_complete(op->tolerated_uncommit_size);
  }
}
...
void ReplicatedBackend::do_repop_reply(OpRequestRef op)
{
  ...
  if (iter != in_progress_ops.end()) {
    ...
    if (ip_op.waiting_for_commit.empty() &&
      ...
      ip_op.on_commit->quorum_complete(ip_op.tolerated_uncommit_size);
    }
  }
}
