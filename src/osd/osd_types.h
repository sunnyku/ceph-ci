...
struct pg_missing_item {
  ...
  void encode(bufferlist& bl, uint64_t features) const {
    ...
    // SERVER_MIMIC and legacy unversioned encoding - a need value of 0'0 is not
    ...
    bool have_server_mimic = HAVE_FEATURE(features, SERVER_MIMIC);

    if (have_recovery_deletes)
      encode(eversion_t(), bl);
    if (have_server_mimic)
