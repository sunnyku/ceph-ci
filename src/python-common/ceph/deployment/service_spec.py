...
class PlacementSpec(object):
    ...
    def validate(self):
        ...
        if self.host_pattern and (self.hosts or self.label or self.all_hosts):
            raise ServiceSpecValidationError('Host patternis mutually exclusive to everything else.')
    ...
    def from_string(cls, arg):
        ...
        hosts = [x for x in strings if x != '*' and 'label:' not in x and not x.startswith('host_pattern:')]
        ...
        if len(host_patterns) > 1:
            raise ServiceSpecValidationError('more than one host_patterns provided: {}'.format(host_patterns))
