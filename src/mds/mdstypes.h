...
struct frag_info_t : public scatter_info_t {
  ...
  std::map<uid_t, int64_t> user_nfiles;     // user files
  std::map<uid_t, int64_t> user_nsubdirs;   // user subdirs
  std::map<gid_t, int64_t> group_nfiles;    // group files
  std::map<gid_t, int64_t> group_nsubdirs;  // group subdirs
...
inline bool operator==(const frag_info_t &l, const frag_info_t &r) {
  return l.version == r.version &&
        ...
        l.user_nfiles == r.user_nfiles &&
        l.group_nfiles == r.group_nfiles &&
        l.user_nsubdirs == r.user_nsubdirs &&
        l.group_nsubdirs == r.group_nsubdirs;
}
inline bool operator!=(const frag_info_t &l, const frag_info_t &r) {
  return !(l == r);
}
...
struct nest_info_t : public scatter_info_t {
  ...
  std::map<uid_t, int64_t> user_rfiles;
  std::map<uid_t, int64_t> user_rsubdirs;
  std::map<gid_t, int64_t> group_rbytes;
  std::map<gid_t, int64_t> group_rfiles;
  std::map<gid_t, int64_t> group_rsubdirs;

  int64_t user_rsize() const {
    int64_t ret = 0;
    for (auto& p : user_rfiles)
      ret += p.second;
    for (auto& p : user_rsubdirs)
      ret += p.second;
    return ret;
  }
  int64_t group_rsize() const {
    int64_t ret = 0;
    for (auto& p : group_rfiles)
      ret += p.second;
    for (auto& p : group_rsubdirs)
      ret += p.second;
    return ret;
  }
...
inline bool operator==(const nest_info_t &l, const nest_info_t &r) {
  return l.version == r.version &&
        ...
        l.user_rfiles == r.user_rfiles &&
        l.group_rfiles == r.group_rfiles &&
        l.user_rsubdirs == r.user_rsubdirs &&
        l.group_rsubdirs == r.group_rsubdirs;
}
inline bool operator!=(const nest_info_t &l, const nest_info_t &r) {
  return !(l == r);
}
...
struct quota_info_t
{
  ...
  std::map<uid_t, int64_t> user_max_files;
  std::map<gid_t, int64_t> group_max_bytes;
  std::map<gid_t, int64_t> group_max_files;
  ...
  bool is_user_enable(uid_t uid)      {
    return user_max_bytes.count(uid)? user_max_bytes[uid] > 0 : false;
  }
  ...
  bool is_group_enable(gid_t gid)      {
    return group_max_bytes.count(gid)? group_max_bytes[gid] > 0 : false;
  }
};
...
inline bool operator==(const quota_info_t &l, const quota_info_t &r) {
  return l.max_bytes == r.max_bytes &&
        ...
        l.user_max_files == r.group_max_files;
}
