...
void MDSMap::add_rank_node_to_consistent_hash_ring(mds_rank_t rank)
{
  ...
  auto iter = std::upper_bound( mds_rank_nodes.begin(), mds_rank_nodes.end(), hash,
...
MDSMap::mds_rank_node MDSMap::get_rank_node_from_ino_in_consistent_hash_ring(inodeno_t ino) const
{
  ...
  int index;
  auto iter = std::lower_bound(mds_rank_nodes.begin(), mds_rank_nodes.end(), hash,
...
vector<inodeno_t> MDSMap::get_inos_from_rank_in_consistent_hash_ring(mds_rank_t rank) const
{
  uint32_t hash = rjhash32(rank);

  /* Do a binary search to get the iterator of the rank */
  auto iter = std::lower_bound(mds_rank_nodes.begin(), mds_rank_nodes.end(), hash,
         [](const mds_rank_node &rank_hash1, uint32_t hash_val) -> bool 
         {
           return rank_hash1.hash < hash_val;
...
mds_rank_t MDSMap::get_successor_of_rank_in_consistent_hash_ring(mds_rank_t rank) const
{
  uint32_t hash = rjhash32(rank);
  struct mds_rank_node last_hash = mds_rank_nodes.back();

  if (hash == last_hash.hash)
    return mds_rank_nodes[0].rank;
  else {
    auto iter = std::upper_bound( mds_rank_nodes.begin(), mds_rank_nodes.end(), hash,
           [](uint32_t hash_val, const mds_rank_node &rank_hash1) -> bool
           {
