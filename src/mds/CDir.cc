...
void CDir::merge(const std::vector<CDir*>& subs, MDSContext::vec& waiters, bool replay)
{
  ...
  for (const auto& dir : subs) {
    ...
    if (dir->fnode.accounted_rstat.version == rstat_version) {
      ...
      if (!dir->fnode.rstat.dirty_from.is_zero())
	rstatdiff.update_dirty_from(dir->fnode.rstat.dirty_from);
    }
...
void CDir::add_dirty_rstat_inode(CInode *in)
{
  ...
  if (rstat_dirty_from.is_zero() ||
    ...
    rstat_dirty_from = pi->rstat.dirty_from;
    cache->mds->locker->mark_dirty_rstat_dirfrag(this);
  }
}
