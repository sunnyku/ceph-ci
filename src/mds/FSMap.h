...
struct MirrorInfo {
  ...
  // client_name/cluster_name variant check
  bool has_peer(std::string_view client_name,
                std::string_view cluster_name) const {
    ClusterInfo cluster_info(client_name, cluster_name);
    for (auto &peer : peers) {
  ...
  void peer_add(std::string_view uuid,
    ...
    peers.emplace(Peer(uuid, ClusterInfo(client_name, cluster_name)));
  }
