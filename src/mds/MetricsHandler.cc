...
void MetricsHandler::remove_session(Session *session) {
  ...
  if (last_updated_seq == 0 || lus == last_updated_seq) {
...
void MetricsHandler::update_rank0() {
  ...
  dout(20) << ": sending metric updates for " << update_client_metrics_map.size()
           << " clients to rank 0 with sequence number " << next_seq
           << ", last updated sequence number " << last_updated_seq << dendl;
  mds->send_message_mds(make_message<MMDSMetrics>(std::move(metrics_message)), (mds_rank_t)0);
}
