...
public:
  ...
  void finish(int r) override {
    const auto& whoami = mds->whoami;
    const auto& incarnation = mds->incarnation;
    ceph_assert(!r);
    auto pos = mds->mdlog->get_journaler()->get_read_pos();
    auto finish = new C_MDS_StandbyReplayRestartFinish(mds, pos);
    mds->mdlog->get_journaler()->reread_head_and_probe(finish);
    dout(1) << " opening purge_queue (async)" << dendl;
    mds->purge_queue.open(nullptr);
