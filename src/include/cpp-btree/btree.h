...
class AlignedAlloc {
  ...
  static constexpr size_t round_up(size_t n) {
    return (n + sizeof(M) - 1) / sizeof(M);
  }
public:
  static void* allocate(Alloc* alloc, size_t n) {
    alloc_t aligned_alloc(*alloc);
    void* p = traits_t::allocate(aligned_alloc, round_up(n));
    assert(reinterpret_cast<uintptr_t>(p) % Alignment == 0 &&
         "allocator does not respect alignment");
    return p;
  }
  static void deallocate(Alloc* alloc, void* p, size_t n) {
    alloc_t aligned_alloc(*alloc);
    traits_t::deallocate(aligned_alloc, static_cast<M*>(p), round_up(n));
  }
};
