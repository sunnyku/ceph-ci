...
//    S3Request.Log()
//
constexpr const char* S3RequestLogAction{"Log"};

int S3RequestLog(lua_State* L) 
{
  ...
  if (store && s) {
    const auto rc = rgw_log_op(store->getRados(), rest, s, ::to_string(s->op_type), olog);
...
struct OwnerMetaTable : public EmptyMetaTable {
  ...
  static int IndexClosure(lua_State* L) {
    ...
    if (strcasecmp(index, "DisplayName") == 0) {
      ...
      create_metatable<UserMetaTable>(L, &(owner->get_id()));
    } else {
      throw_unknown_field(index, TableName());
    }
    return ONE_RETURNVAL;
...
struct BucketMetaTable : public EmptyMetaTable {
  ...
  static int IndexClosure(lua_State* L) {
    ...
    if (strcasecmp(index, "Tenant") == 0) {
      ...
      create_metatable<QuotaMetaTable>(L, &(bucket->get_info().quota));
    } else if (strcasecmp(index, "PlacementRule") == 0) {
      create_metatable<PlacementRuleMetaTable>(L, &(bucket->get_info().placement_rule));
    } else if (strcasecmp(index, "User") == 0) {
      create_metatable<UserMetaTable>(L, &(bucket->get_info().owner));
    } else {
      throw_unknown_field(index, TableName());
    }
    return ONE_RETURNVAL;
...
struct GrantMetaTable : public EmptyMetaTable {
  ...
  static int IndexClosure(lua_State* L) {
    ...
    if (strcasecmp(index, "Type") == 0) {
      ...
      create_metatable<UserMetaTable>(L, const_cast<rgw_user*>(&(grant->get_id())));
...
struct GrantsMetaTable : public EmptyMetaTable {
  ...
  static int IndexClosure(lua_State* L) {
    ...
    if (it == map->end()) {
      ...
      create_metatable<GrantMetaTable>(L, &(it->second));
    }
    return ONE_RETURNVAL;
  }
  ...
  static int stateless_iter(lua_State* L) {
    ...
    lua_pushstring(L, next_it->first);
    create_metatable<GrantMetaTable>(L, &(next_it->second));
    // return key, value
...
struct ACLMetaTable : public EmptyMetaTable {
  ...
  static int IndexClosure(lua_State* L) {
    ...
    if (strcasecmp(index, "Owner") == 0) {
      create_metatable<OwnerMetaTable>(L, &(acl->get_owner()));
    } else if (strcasecmp(index, "Grants") == 0) {
      create_metatable<GrantsMetaTable>(L, &(acl->get_acl().get_grant_map()));
    } else {
      throw_unknown_field(index, TableName());
    }
    return ONE_RETURNVAL;
...
struct PoliciesMetaTable : public EmptyMetaTable {
  ...
  static int IndexClosure(lua_State* L) {
    ...
    if (index >= (int)policies->size() || index < 0) {
      ...
      create_metatable<PolicyMetaTable>(L, &((*policies)[index]));
    }
    return ONE_RETURNVAL;
  }
  ...
  static int stateless_iter(lua_State* L) {
    ...
    if (next_it >= policies->size()) {
      ...
      lua_pushinteger(L, next_it);
      create_metatable<PolicyMetaTable>(L, &((*policies)[next_it]));
      // return key, value
    }
...
struct CopyFromMetaTable : public EmptyMetaTable {
  ...
  static int IndexClosure(lua_State* L) {
    ...
    if (strcasecmp(index, "Tenant") == 0) {
      ...
      create_metatable<ObjectMetaTable>(L, s->src_object);
    } else {
      throw_unknown_field(index, TableName());
    }
    return ONE_RETURNVAL;
...
struct S3RequestMetaTable : public EmptyMetaTable {
  static std::string TableName() {return "S3Request";}
  static std::string Name() {return TableName() + "Meta";}
  ...
  static int IndexClosure(lua_State* L) {
    ...
    if (strcasecmp(index, "RGWOp") == 0) {
      lua_pushstring(L, to_string(s->op_type));
      ...
      create_metatable<StringMapMetaTable<>>(L, &(s->generic_attrs));
    } else if (strcasecmp(index, "Response") == 0) {
      create_metatable<ResponseMetaTable>(L, &(s->err));
      ...
      create_metatable<BucketMetaTable>(L, s->bucket);
    } else if (strcasecmp(index, "Object") == 0) {
      create_metatable<ObjectMetaTable>(L, s->object);
      ...
      create_metatable<OwnerMetaTable>(L, &(s->owner));
    } else if (strcasecmp(index, "ZoneGroup") == 0) {
      create_metatable<ZoneGroupMetaTable>(L, s);
    } else if (strcasecmp(index, "UserACL") == 0) {
      create_metatable<ACLMetaTable>(L, s->user_acl);
    } else if (strcasecmp(index, "BucketACL") == 0) {
      create_metatable<ACLMetaTable>(L, s->bucket_acl);
    } else if (strcasecmp(index, "ObjectACL") == 0) {
      create_metatable<ACLMetaTable>(L, s->object_acl);
    } else if (strcasecmp(index, "Environment") == 0) {
        create_metatable<StringMapMetaTable<rgw::IAM::Environment>>(L, &(s->env));
    } else if (strcasecmp(index, "Policy") == 0) {
      // TODO: create a wrapper to std::optional
      if (!s->iam_policy) {
        lua_pushnil(L);
      } else {
        create_metatable<PolicyMetaTable>(L, s->iam_policy.get_ptr());
      }
    } else if (strcasecmp(index, "UserPolicies") == 0) {
        create_metatable<PoliciesMetaTable>(L, &(s->iam_user_policies));
    } else if (strcasecmp(index, "RGWId") == 0) {
      lua_pushstring(L, s->host_id);
    } else if (strcasecmp(index, "HTTP") == 0) {
        create_metatable<HTTPMetaTable>(L, &(s->info));
      ...
      create_metatable<StringMapMetaTable<RGWObjTags::tag_map_t>>(L, &(s->tagset.get_tags()));
    } else {
      throw_unknown_field(index, TableName());
    }
    return ONE_RETURNVAL;
    ...
    const std::string& script) 
{
  ...
  create_metatable<S3RequestMetaTable>(L, s, true);
  
  // add the ops log action
  lua_getglobal(L, S3RequestMetaTable::TableName().c_str());
  ceph_assert(lua_istable(L, -1));
  lua_pushstring(L, S3RequestLogAction);
  lua_pushlightuserdata(L, store);
  lua_pushlightuserdata(L, rest);
  lua_pushlightuserdata(L, olog);
  lua_pushlightuserdata(L, s);
  lua_pushcclosure(L, S3RequestLog, FOUR_UPVALS);
  lua_rawset(L, -3);
