...
public:
  RGWBucketSyncSingleEntryCR(RGWDataSyncEnv *_sync_env,
    ...
    ss << bucket_shard_str{bs} << "/" << key
	   << "[" << inl_value_or(versioned_epoch, 0) << "]";
        ...
        yield {
          ...
          if (op == CLS_RGW_OP_ADD ||
            ...
            ldout(sync_env->cct, 5) << "bucket sync: sync obj: " << sync_env->source_zone << "/" << bucket_info->bucket << "/" << key
									<< "[" << inl_value_or(versioned_epoch, 0) << "]" << dendl;
            logger.log("fetch");
            ...
            logger.log("remove");
            call(new RGWRemoveObjCR(sync_env->async_rados, sync_env->store, sync_env->source_zone, *bucket_info, key, versioned, inl_value_or(versioned_epoch, 0), NULL, NULL, false, &timestamp));
            ...
            call(new RGWRemoveObjCR(sync_env->async_rados, sync_env->store, sync_env->source_zone, *bucket_info, key, versioned,
									inl_value_or(versioned_epoch, 0), &owner.id, &owner.display_name, true, &timestamp));
          }
        }
