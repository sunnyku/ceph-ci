...
class Manager {
  ...
  bool queue_owned(const std::string& queue_name, spawn::yield_context yield) {
    ...
    if (ret == -ENOENT) {
      ldout(cct, 10) << "INFO: check ownership of queue: " << queue_name << " - already deleted" << dendl;
      return false;
    }
    if (ret < 0) {
      ldout(cct, 1) << "ERROR: check ownership of queue: " << queue_name << ". error: " << ret << dendl;
      return true; 
    }
    return true;
  }
  ...
  void cleanup_queue(const std::string& queue_name, spawn::yield_context yield) {
    while (queue_owned(queue_name, yield)) {
