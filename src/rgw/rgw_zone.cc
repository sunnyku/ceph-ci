...
int RGWSystemMetaObj::read_default(RGWDefaultSystemMetaObjInfo& default_info, const string& oid)
{
  ...
  int ret = ceph::from_error_code(sysobj.rop().read(&bl, null_yield));
  if (ret < 0)
    return ret;
...
int RGWRealm::notify_zone(bufferlist& bl)
{
  ...
  int ret = ceph::from_error_code(sysobj.wn().notify(bl, std::nullopt, nullptr, null_yield));
  if (ret < 0) {
    return ret;
  }
  return 0;
...
int RGWPeriod::delete_obj()
{
  ...
  for (epoch_t e = 1; e <= epoch; e++) {
    ...
    int ret = ceph::from_error_code(sysobj.wop().remove(null_yield));
    if (ret < 0) {
...
int RGWPeriod::update()
{
  ...
  int ret = ceph::from_error_code(zone_svc->list_zonegroups(zonegroups));
  if (ret < 0) {
...
int RGWZoneParams::fix_pool_names()
{
  ...
  int r = ceph::from_error_code(zone_svc->list_zones(zones));
  if (r < 0) {
...
int RGWZoneParams::create(bool exclusive)
{
  ...
  auto obj_ctx = sysobj_svc->init_obj_ctx();
  auto sysobj = sysobj_svc->get_obj(obj_ctx, obj);
  int r = ceph::from_error_code(sysobj.rop().stat(null_yield));
  if (r < 0) {
