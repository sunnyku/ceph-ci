  ...
  size_t write_data(const char* buf, size_t len) override {
    ...
    if (ec) {
      ...
      if (ec==boost::asio::error::broken_pipe) {
        ...
        get_lowest_layer(stream).socket().shutdown(tcp::socket::shutdown_both, ec_ignored);
      }
...
class AsioFrontend {
  ...
  ceph::timespan request_timeout = std::chrono::milliseconds(65000);
#ifdef WITH_RADOSGW_BEAST_OPENSSL
...
// Setting global timeout
  ...
  if (timeout != config.end()) {
    ...
    const auto ec = std::from_chars(timeout->second.data(), timeout->second.data() + timeout->second.size(), timeout_number, 10);
    if(ec.ec == std::errc()) {
      request_timeout = std::chrono::milliseconds(timeout_number);
    } else {
      lderr(ctx()) << "failed to parse request_timeout_ms setting it to the default value" << dendl;
    }
  } 
#ifdef WITH_RADOSGW_BEAST_OPENSSL
  int r = init_ssl();
