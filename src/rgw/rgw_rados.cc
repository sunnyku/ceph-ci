...
void RGWRados::gen_rand_obj_instance_name(rgw_obj *target_obj)
{
  ...
  /* don't want it to get url escaped, no underscore for instance name due to the way we encode the raw keys */
  target_obj->key.set_instance(gen_rand_alphanumeric_no_underscore(cct, obj_instance_len));
}
