...
#include <cpp_redis/cpp_redis> //ugur-wb
...
class RGWRadosPutObj : public RGWHTTPStreamRWRequest::ReceiveCB
{
  ...
  RGWGetDataCB* client_cb; //ugur-wb
public:
  ...
  int handle_data(bufferlist& bl, bool *pause) override {
    if (progress_cb) {
      ldout(cct, 0) << "process_cb is called" << dendl;
...
int RGWRados::Object::Read::fetch_from_backend(RGWGetDataCB *cb, string userid, string bucket_name, string obj_name, req_state * const s){
  RGWRados *store = source->get_store();
  CephContext *cct = store->ctx();
  bool done = false;
  RGWObjectCtx& obj_ctx = source->get_ctx();
  rgw_obj& obj = source->get_obj();
  ldout(cct, 0) << "ugur access s value " << s->bucket_name << " obj_si "<< s->obj_size << dendl;
//  RGWCreateBucket *op;
//  RGWHandler *h;
//  op->init(store, s,h);
//  ldout(cct, 0) << "ugur access s init " << dendl;
//  op->execute();
//  ldout(cct, 0) << "ugur access s execute " << dendl;
 
  
  
  int r = store->fetch_remote(store, userid, bucket_name, obj_name, cb, obj_ctx );
}
...
//    if (r == -ENOENT) {
    if (true){
      ...
      string url ="http://" + cct->_conf->rgw_host;
      string etag;
      ...
      ret = store->get_bucket_info(obj_ctx2, "", dest_bucket_name, bucket_info, NULL, &bucket_attrs); 
...
int RGWRados::get_s3_credentials(RGWRados *store, string userid, RGWAccessKey& s3_key, CephContext *cct){
    RGWObjVersionTracker objv_tracker;
    RGWObjectCtx obj_ctx(store);
    RGWUserInfo info;
...
int RGWRados::fetch_remote(RGWRados *store, string userid, string dest_bucket_name, string dest_obj_name,RGWGetDataCB *client_cb, RGWObjectCtx& obj_ctx ){
  ldout(cct, 0) << "RGWRados::fetch_remote() userid " << userid
                << " bucket name " << dest_bucket_name
                << "object name"  << dest_obj_name << dendl;
  ...
  using namespace rgw::putobj;
  AioThrottle aio(cct->_conf->rgw_put_obj_min_window_size);
  AtomicObjectProcessor processor(&aio, store, dest_bucket_info, dest_bucket_info.owner,
  ...
  endpoints.push_back("http://" + cct->_conf->rgw_backend_address);
  
  RGWRESTConn *conn = new RGWRESTConn(cct, store, tenant_id, endpoints, accesskey);
  RGWRESTStreamRWRequest *in_stream_req;
  boost::optional<RGWPutObj_Compress> compressor;
  CompressorRef plugin;
...
/*string *petag = nullptr;
  if (petag) {
    const auto iter = dest_state->attrset.find(RGW_ATTR_ETAG);
    if (iter != dest_state->attrset.end()) {
      *petag = iter->second.to_str();
       dout(10)  << "after 2get_obj  ugur petag " << petag << dendl;
    }
  }
*/
...
int RGWRados::set_key(string key, string timeStr, string location, string owner, uint64_t obj_size, string etag){
//int RGWRados::set_key(string key, string timeStr, string owner, string loc0, string loc1, string flag0, string flag1){
...
void RGWRados::get_key(string key, directory_values &dir_val){
  ...
  client.hmget(key, fields, [&owner, &time, &location, &etag, &sizeStr](cpp_redis::reply &reply){
	//std::cout << reply << '\n';
	owner = reply.as_array()[0];
	time = reply.as_array()[1];
	location = reply.as_array()[2];
	sizeStr = reply.as_array()[3];
	etag = reply.as_array()[4];
  });
...
int RGWRados::get_key(directory_values &dir_val){
  dout(10) << __func__ << " redis get key ="  <<dir_val.key << dendl;
  dir_val.owner="testuser";
  cpp_redis::client client;
  client.connect();
  cpp_redis::reply answer;
  client.send({"get", dir_val.key}, [&answer](cpp_redis::reply &reply) {
                answer = std::move(reply);
                });
        client.sync_commit();
        dout(10) << __func__ << " redis answer ="  <<answer << dendl;
        if (answer.is_null()){
          dir_val.location = "datalake";
        } else if (answer.is_string()){
           dir_val.location = answer.as_string();
        }
	return 0;
}
*/
