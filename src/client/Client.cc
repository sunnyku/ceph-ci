...
size_t Client::_vxattrcb_user_quota(Inode *in, char *val, size_t size, const char *name)
{
  if (name) {
    ...
    return snprintf(val, size, "%lld used_bytes:%lld",
           (long long int)in->quota.user_max_files[pw->pw_uid], user_rbytes);
  } else {
    int r=0;
    for (auto& p : in->quota.user_max_bytes){
      if(p.second > 0){
        ...
        len = snprintf(buf, sizeof(buf), "user_id:%lld max_bytes:%lld used_bytes:%lld ",
...
size_t Client::_vxattrcb_user_quota_max_bytes(Inode *in, char *val, size_t size, const char *name)
{
  if (name) {
    ...
    int64_t user_rbytes = in->rstat.user_rbytes.count(pw->pw_uid)? in->rstat.user_rbytes[pw->pw_uid] : 0;
    return snprintf(val, size, "%lld used_bytes:%lld",
           (long long int)in->quota.user_max_bytes[pw->pw_uid], user_rbytes);
  } else {
    int r=0;
    for (auto& p : in->quota.user_max_bytes){
      if(p.second > 0){
...
size_t Client::_vxattrcb_user_quota_max_files(Inode *in, char *val, size_t size, const char *name)
{
  if (name) {
    struct passwd *pw = getpwnam(name);
    if (!pw)
      return -ENODATA;
    return snprintf(val, size, "%lld", (long long int)in->quota.user_max_files[pw->pw_uid]);
  } else {
    int r=0;
    for (auto& p : in->quota.user_max_files){
      if(p.second > 0)
        r += snprintf(val + r, size - r, "user_id:%lld max_files:%lld ",
          (long long int)p.first, (long long int)p.second);
    }
    return r;
  }
}
...
size_t Client::_vxattrcb_group_quota(Inode *in, char *val, size_t size, const char *name)
{
  if (name) {
    ...
    return snprintf(val, size, "%lld used_bytes:%lld",
      (long long int)in->quota.group_max_bytes[data->gr_gid], group_rbytes);
  } else {
    int r=0;
    for (auto& p : in->quota.group_max_bytes){
      if(p.second > 0){
        ...
        len = snprintf(buf, sizeof(buf), "group_id:%lld max_bytes:%lld used_bytes:%lld ",
...
size_t Client::_vxattrcb_group_quota_max_files(Inode *in, char *val, size_t size, const char *name)
{
  if (name) {
    struct group * data = getgrnam(name);
    if (!data)
      return -ENODATA;
    return snprintf(val, size, "%lld", (long long int)in->quota.group_max_files[data->gr_gid]);
  } else {
    int r=0;
    for (auto& p : in->quota.group_max_files){
      if(p.second > 0)
        r += snprintf(val + r, size - r, "group_id:%lld max_files:%lld ",
          (long long int)p.first, (long long int)p.second);
    }
    return r;
  }
...
size_t Client::_vxattrcb_dir_user_rfiles(Inode *in, char *val, size_t size, const char *name)
{
  size_t r = 0;
  for (auto& p : in->rstat.user_rfiles)
    r += snprintf(val, size, "id:%llu rfiles:%llu ", (unsigned long long)p.first, (unsigned long long)p.second);
  return r;
}
size_t Client::_vxattrcb_dir_user_rsubdirs(Inode *in, char *val, size_t size, const char *name)
{
  size_t r = 0;
  for (auto& p : in->rstat.user_rsubdirs)
    r += snprintf(val, size, "id:%llu rsubdirs:%llu ", (unsigned long long)p.first, (unsigned long long)p.second);
  return r;
}
...
size_t Client::_vxattrcb_dir_group_rfiles(Inode *in, char *val, size_t size, const char *name)
{
  size_t r = 0;
  for (auto& p : in->rstat.group_rfiles)
    r += snprintf(val, size, "id:%llu rfiles:%llu ", (unsigned long long)p.first, (unsigned long long)p.second);
  return r;
}
size_t Client::_vxattrcb_dir_group_rsubdirs(Inode *in, char *val, size_t size, const char *name)
{
  size_t r = 0;
  for (auto& p : in->rstat.group_rsubdirs)
    r += snprintf(val, size, "id:%llu rsubdirs:%llu ", (unsigned long long)p.first, (unsigned long long)p.second);
  return r;
}
...
const Client::VXattr Client::_dir_vxattrs[] = {
  ...
  XATTR_NAME_CEPH2(dir, user_rfiles, VXATTR_RSTAT),
  XATTR_NAME_CEPH2(dir, user_rsubdirs, VXATTR_RSTAT),
  XATTR_NAME_CEPH2(dir, group_rbytes, VXATTR_RSTAT),
  XATTR_NAME_CEPH2(dir, group_rfiles, VXATTR_RSTAT),
  XATTR_NAME_CEPH2(dir, group_rsubdirs, VXATTR_RSTAT),
  {
    name: "ceph.quota",
  ...
  XATTR_USER_QUOTA_FIELD(user_quota, max_files),
  {
  ...
  XATTR_GROUP_QUOTA_FIELD(group_quota, max_files),
  /* end */
...
bool Client::is_user_quota_files_exceeded(Inode *in, const UserPerm& perms, bool flag)
{
  uid_t tmp_uid = flag? perms.uid() : in->uid;

  return check_user_quota_condition(in, tmp_uid,
      [&tmp_uid](Inode &in) {
        int64_t user_maxfiles = in.quota.user_max_files.count(tmp_uid)? in.quota.user_max_files[tmp_uid] : 0;
        int64_t user_rfiles = in.rstat.user_rfiles.count(tmp_uid)? in.rstat.user_rfiles[tmp_uid] : 0;
        return (user_maxfiles > 0 && user_rfiles >= user_maxfiles);
      });
}
...
bool Client::is_group_quota_files_exceeded(Inode *in, const UserPerm& perms, bool flag)
{
  gid_t tmp_gid = flag? perms.gid() : in->gid;

  return check_group_quota_condition(in, tmp_gid,
      [&tmp_gid](Inode &in) {
        int64_t group_maxfiles = in.quota.group_max_files.count(tmp_gid)? in.quota.group_max_files[tmp_gid] : 0;
        int64_t group_rfiles = in.rstat.group_rfiles.count(tmp_gid)? in.rstat.group_rfiles[tmp_gid] : 0;
        return (group_maxfiles > 0 && group_rfiles >= group_maxfiles);
      });
}
