...
cdef extern from "rados/librados.h" nogil:
    ...
    int rados_decode_notify_msg(char *reply_buffer, size_t reply_buffer_len, notify_ack_t **acks, size_t *nr_acks, notify_timeout_t **timeouts, size_t *nr_timeouts)
    void rados_notify_buffer_free(notify_ack_t *acks, size_t nr_acks, notify_timeout_t *timeouts)
...
returned %d, but should return zero on success." % (self.name, ret))
    ...
    @requires(('obj', str), ('oncomplete', opt(Callable)), ('msg', str), ('timeout_ms', int))
    def aio_notify(self, obj, oncomplete, msg='', timeout_ms=5000):
        """
        Asynchronously send a rados notification to an object

        :param obj: the name of the object to notify
        :type obj: str
        :param oncomplete: what to do when notification acknowledgement is received
                           or notification times out
        :type oncomplete: completion
        :param msg: optional message to send in the notification
        :type msg: str
        :param timeout_ms: notify timeout (in ms)
        ...
        def oncomplete_(completion_v):
            ...
            return_value = _completion_v.get_return_value()
            if return_value == 0:
                return_value = rados_decode_notify_msg(reply, replylen, &acks, &nr_acks, &timeouts, &nr_timeouts)
            if return_value == 0:
                ack_list = [(ack.notifier_id, ack.cookie, '' if not ack.payload else ack.payload) for ack in acks[:nr_acks]]
                timeout_list = [(timeout.notifier_id, timeout.cookie) for timeout in timeouts[:nr_timeouts]]
                rados_notify_buffer_free(acks, nr_acks, timeouts)
