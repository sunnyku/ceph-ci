...
class Module(MgrModule):
    ...
    COMMANDS = [
        ...
        {
            "cmd": "telemetry show",
            "desc": "Show last report or report to be send",
            "perm": "r"
        }
    ]
    ...
    def parse_timestamp(timestamp):
        return datetime.strptime(timestamp, '%Y-%m-%d %I:%M:%S.%f')
    ...
    def compile_report(self):
        ...
        for option in ['description', 'contact', 'organization']:
            ...
            if self.config[option]:
                report[option] = self.config[option]
    ...
    def serve(self):
        ...
        while self.run:
            if not self.str_to_bool(self.config['enabled']):
