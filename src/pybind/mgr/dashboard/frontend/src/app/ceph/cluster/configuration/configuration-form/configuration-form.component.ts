...
export class ConfigurationFormComponent implements OnInit {
  ...
  getType(type: string): any {
    const knownTypes = [
      { name: 'uint64_t', inputType: 'number', humanReadable: 'Integer value' },
      { name: 'int64_t', inputType: 'number', humanReadable: 'Integer value' },
      { name: 'size_t', inputType: 'number', humanReadable: 'Integer value (size)' },
      { name: 'secs', inputType: 'number', humanReadable: 'Integer value (secs)' },
      { name: 'double', inputType: 'number', humanReadable: 'Decimal value' },
      { name: 'std::string', inputType: 'text', humanReadable: 'Text' },
      { name: 'entity_addr_t', inputType: 'text', humanReadable: 'IPv4 or IPv6 address' },
      { name: 'uuid_d', inputType: 'text', humanReadable: 'UUID' },
      { name: 'bool', inputType: 'checkbox', humanReadable: 'Boolean value' }
    ];
  ...
  getPatternHelpText(type: string): string {
    ...
    if (numberTypes.includes(type)) {
      return 'The entered value needs to be a number.';
    } else if (type === 'double') {
      ...
      return 'The entered value is not a valid UUID, e.g.: 67dcac9f-2c03-4d6c-b7bd-1210b3a259a8';
    }
  }
  ...
  getValidators(configOption: any): ValidatorFn[] {
    ...
    if (numberTypes.includes(configOption.type)) {
      ...
      if (configOption.type === 'double') {
        ...
        validators.push(CdValidators.number());
      }
      return validators;
