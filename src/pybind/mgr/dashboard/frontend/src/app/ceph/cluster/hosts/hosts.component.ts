...
export class HostsComponent extends ListWithDetails implements OnInit {
    ...
    this.tableActions = [
      {
        ...
        disable: (selection: CdTableSelection) => this.getDisable('create', selection).state,
        disableDesc: (selection: CdTableSelection) => this.getDisable('create', selection).desc
      },
      {
        ...
        disable: (selection: CdTableSelection) => this.getDisable('edit', selection).state,
        disableDesc: (selection: CdTableSelection) => this.getDisable('edit', selection).desc
      },
      {
        ...
        disable: (selection: CdTableSelection) => this.getDisable('delete', selection).state,
        disableDesc: (selection: CdTableSelection) => this.getDisable('delete', selection).desc
      }
    ];
  }
  ...
  ): { state: boolean; desc?: string } {
    if (action === 'delete' || action === 'edit') {
      if (!selection?.hasSelection) {
        return { state: true, desc: undefined };
      }
      if (!selection.first().sources.orchestrator) {
        return { state: true, desc: this.messages.nonOrchHost };
      }
    }
    const desc = this.orchService.getTableActionDisableDesc(
      this.orchStatus,
      this.actionOrchFeatures[action]
    );
    return { state: !!desc, desc: desc };
  }
