...
"""
        ...
        if service_name:
            if preview:
                out = self.preview_osdspecs(osdspec_name=service_name)
                return HandleCommandResult(stdout=print_preview(out, format))
    ...
    def _apply_specs(self, specs, format=format):
        completion = self.apply_drivegroups(specs)
        self._orchestrator_wait([completion])
        raise_if_exception(completion)
        return HandleCommandResult(stdout=to_format(completion.result_str(), format))
