...
class TestOrchestrator(MgrModule, orchestrator.Orchestrator):
    ...
    def update_mgrs(self, spec):
        assert not spec.placement.nodes or len(spec.placement.nodes) == spec.placement.count
        assert all([isinstance(h, str) for h in spec.placement.nodes])

    @deferred_write("update_mons")
    def update_mons(self, spec):
        assert not spec.placement.nodes or len(spec.placement.nodes) == spec.placement.count
        assert all([isinstance(h[0], str) for h in spec.placement.nodes])
        assert all([isinstance(h[1], str) or h[1] is None for h in spec.placement.nodes])
