import errno
import logging
import time
from threading import Lock

from .state_transition import ActionType, PolicyAction, Transition, \
    State, StateTransition
from ..exception import MirrorException

log = logging.getLogger(__name__)

class DirectoryState(object):
    def __init__(self, instance_id=None, mapped_time=None):
        self.instance_id = instance_id
        self.mapped_time = mapped_time
        self.state = State.STATE_UNASSOCIATED
        self.stalled = False
        self.transition = Transition(ActionType.ACTION_TYPE_NONE)
        self.next_state = None
        self.purging = False

    def __str__(self):
        return f'[instance_id={self.instance_id}, mapped_time={self.mapped_time},'\
            ' state={self.state}, transition={self.transition}, next_state={self.next_state},'\
            ' purging={self.purging}]'

class Policy(object):
    def __init__(self):
        self.dir_states = {}
        self.instance_to_dir_map = {}
        self.dead_instances = []
        self.lock = Lock()

    @staticmethod
    def is_instance_action(action_type):
        return action_type in (ActionType.ACTION_TYPE_ACQUIRE,
                               ActionType.ACTION_TYPE_RELEASE)

    def is_dead_instance(self, instance_id):
        return instance_id in self.dead_instances

    def is_state_scheduled(self, dir_state, state):
        return dir_state.state == state or dir_state.next_state == state

    def is_shuffling(self, dir_name):
        log.debug(f'is_shuffling: {dir_name}')
        return self.is_state_scheduled(self.dir_states[dir_name], State.STATE_SHUFFLING)

    def can_shuffle_dir(self, dir_name):
        """Right now, shuffle directories only based on idleness. Later, we
        probably want to avoid shuffling images that were recently shuffled.
        """
        log.debug(f'can_shuffle_dir: {dir_name}')
        dir_state = self.dir_states[dir_name]
        return StateTransition.is_idle(dir_state.state)

    def set_state(self, dir_state, state, ignore_current_state=False):
        if not ignore_current_state and dir_state.state == state:
            return False
        elif StateTransition.is_idle(dir_state.state):
            dir_state.state = state
            dir_state.next_state = None
            dir_state.transition = StateTransition.transit(
                dir_state.state, dir_state.transition.action_type)
            return True
        dir_state.next_state = state
        return False

    def init(self, dir_mapping):
        with self.lock:
            for dir_name, dir_map in dir_mapping.items():
                instance_id = dir_map['instance_id']
                if instance_id:
                    if not instance_id in self.instance_to_dir_map:
                        self.instance_to_dir_map[instance_id] = []
                    self.instance_to_dir_map[instance_id].append(dir_name)
                self.dir_states[dir_name] = DirectoryState(instance_id, dir_map['last_shuffled'])
                dir_state = self.dir_states[dir_name]
                state = State.STATE_INITIALIZING if instance_id else State.STATE_ASSOCIATING
                if instance_id:
                    purging = dir_mapping.get('purging', False)
                    if purging:
                        dir_state.purging = True
                        state = State.STATE_DISASSOCIATING
                    else:
                        state = State.STATE_INITIALIZING
                else:
                    state = State.STATE_ASSOCIATING
                log.debug(f'starting state: {dir_name} {state}')
                self.set_state(dir_state, state)
                log.debug(f'init dir_state: {dir_state}')

    def lookup(self, dir_name):
        log.debug(f'looking up {dir_name}')
        with self.lock:
            dir_state = self.dir_states.get(dir_name, None)
            if dir_state:
                return {'instance_id': dir_state.instance_id,
                        'mapped_time': dir_state.mapped_time,
                        'purging': dir_state.purging}
            return None

    def map(self, dir_name, dir_state):
        log.debug(f'mapping {dir_name}')
        min_instance_id = None
        current_instance_id = dir_state.instance_id
        if current_instance_id and not self.is_dead_instance(current_instance_id):
            return True
        if self.is_dead_instance(current_instance_id):
            self.unmap(dir_name, dir_state)
        for instance_id, dir_names in self.instance_to_dir_map.items():
            if self.is_dead_instance(instance_id):
                continue
            if not min_instance_id or len(dir_names) < len(self.instance_to_dir_map[min_instance_id]):
                min_instance_id = instance_id
        if not min_instance_id:
            log.debug(f'instance unavailable for {dir_name}')
            return False
        log.debug(f'dir_name {dir_name} maps to instance {min_instance_id}')
        dir_state.instance_id = min_instance_id
        dir_state.mapped_time = time.time()
        self.instance_to_dir_map[min_instance_id].append(dir_name)
        return True

    def unmap(self, dir_name, dir_state):
        instance_id = dir_state.instance_id
        log.debug(f'unmapping {dir_name} from instance {instance_id}')
        self.instance_to_dir_map[instance_id].remove(dir_name)
        dir_state.instance_id = None
        dir_state.mapped_time = None
        if self.is_dead_instance(instance_id) and not self.instance_to_dir_map[instance_id]:
            self.instance_to_dir_map.pop(instance_id)
            self.dead_instances.remove(instance_id)

    def shuffle(self, dirs_per_instance, include_stalled_dirs):
        log.debug(f'directories per instance: {dirs_per_instance}')
        shuffle_dirs = []
        for instance_id, dir_names in self.instance_to_dir_map.items():
            cut_off = len(dir_names) - dirs_per_instance
            if cut_off > 0:
                for dir_name in dir_names:
                    if cut_off == 0:
                        break
                    if self.is_shuffling(dir_name):
                        cut_off -= 1
                    elif self.can_shuffle_dir(dir_name):
                        cut_off -= 1
                        shuffle_dirs.append(dir_name)
        if include_stalled_dirs:
            for dir_name, dir_state in self.dir_states.items():
                if dir_state.stalled:
                    log.debug(f'{dir_name} is stalled: {dir_state} -- trigerring kick')
                    dir_state.stalled = False
                    shuffle_dirs.append(dir_name)
        return shuffle_dirs

    def execute_policy_action(self, dir_name, dir_state, policy_action):
        log.debug(f'executing for directory {dir_name} policy_action {policy_action}')

        done = True
        if policy_action == PolicyAction.POLICY_ACTION_MAP:
            done = self.map(dir_name, dir_state)
        elif policy_action == PolicyAction.POLICY_ACTION_UNMAP:
            self.unmap(dir_name, dir_state)
        elif policy_action == PolicyAction.POLICY_ACTION_REMOVE:
            if dir_state.state == State.STATE_UNASSOCIATED:
                self.dir_states.pop(dir_name)
        else:
            raise Exception()
        return done

    def start_action(self, dir_name):
        log.debug(f'start action: {dir_name}')
        with self.lock:
            dir_state = self.dir_states.get(dir_name, None)
            if not dir_state:
                raise Exception()
            log.debug(f'dir_state: {dir_state}')
            if dir_state.transition.start_policy_action:
                stalled = not self.execute_policy_action(dir_name, dir_state,
                                                         dir_state.transition.start_policy_action)
                if stalled:
                    dir_state.stalled = True
                    log.debug(f'state machine stalled')
                    return ActionType.ACTION_TYPE_NONE
            return dir_state.transition.action_type

    def finish_action(self, dir_name, r):
        log.debug(f'finish action {dir_name} r={r}')
        with self.lock:
            dir_state = self.dir_states.get(dir_name, None)
            if not dir_state:
                raise Exception()
            if r < 0 and (not Policy.is_instance_action(dir_state.transition.action_type) or
                          not dir_state.instance_id or
                          not dir_state.instance_id in self.dead_instances):
                return True
            log.debug(f'dir_state: {dir_state}')
            finish_policy_action = dir_state.transition.finish_policy_action
            dir_state.transition = StateTransition.transit(
                dir_state.state, dir_state.transition.action_type)
            log.debug(f'transitioned to dir_state: {dir_state}')
            if dir_state.transition.final_state:
                log.debug('reached final state')
                dir_state.state = dir_state.transition.final_state
                dir_state.transition = Transition(ActionType.ACTION_TYPE_NONE)
                log.debug(f'final dir_state: {dir_state}')
            if StateTransition.is_idle(dir_state.state) and dir_state.next_state:
                self.set_state(dir_state, dir_state.next_state)
            pending = not dir_state.transition.action_type == ActionType.ACTION_TYPE_NONE
            if finish_policy_action:
                self.execute_policy_action(dir_name, dir_state, finish_policy_action)
            return pending

    def add_directory(self, dir_name):
        log.debug(f'adding dir_name {dir_name}')
        rd = {'schedule': False,
              'exists': False
              }
        with self.lock:
            if dir_name in self.dir_states:
                rd['exists'] = True
                return rd
            self.dir_states[dir_name] = DirectoryState()
            dir_state = self.dir_states[dir_name]
            log.debug(f'add dir_state: {dir_state}')
            if dir_state.state == State.STATE_INITIALIZING:
                return rd
            schedule = self.set_state(dir_state, State.STATE_ASSOCIATING)
            rd['schedule'] = schedule
            return rd

    def remove_directory(self, dir_name):
        log.debug(f'removing dir_name {dir_name}')
        with self.lock:
            dir_state = self.dir_states.get(dir_name, None)
            if not dir_state:
                return False
            log.debug(f'removing dir_state: {dir_state}')
            dir_state.purging = True
            return self.set_state(dir_state, State.STATE_DISASSOCIATING)

    def add_instances_initial(self, instance_ids):
        """Take care of figuring out instances which no longer exist
        and remove them. This is to be done only once on startup to
        identify instances which were previously removed but directories
        are still mapped (on-disk) to them.
        """
        for instance_id in instance_ids:
            if not instance_id in self.instance_to_dir_map:
                self.instance_to_dir_map[instance_id] = []
        dead_instances = []
        for instance_id, _ in self.instance_to_dir_map.items():
            if not instance_id in instance_ids:
                dead_instances.append(instance_id)
        if dead_instances:
            self._remove_instances(dead_instances)

    def add_instances(self, instance_ids, initial_update=False):
        log.debug(f'adding instances: {instance_ids} initial_update {initial_update}')
        with self.lock:
            if initial_update:
                self.add_instances_initial(instance_ids)
            else:
                nr_instances = len(self.instance_to_dir_map)
                nr_dead_instances = len(self.dead_instances)
                if nr_instances > 0:
                    # adjust dead instances
                    nr_instances -= nr_dead_instances
                include_stalled_dirs = nr_instances == 0
                for instance_id in instance_ids:
                    if not instance_id in self.instance_to_dir_map:
                        self.instance_to_dir_map[instance_id] = []
                dirs_per_instance = int(len(self.dir_states) /
                                        (len(self.instance_to_dir_map) - nr_dead_instances))
                if dirs_per_instance == 0:
                    dirs_per_instance += 1
                shuffle_dirs = []
                # super set of directories which are candidates for shuffling -- choose
                # those which can be shuffle rightaway (others will be shuffled when
                # they reach idle state).
                shuffle_dirs_ss = self.shuffle(dirs_per_instance, include_stalled_dirs)
                if include_stalled_dirs:
                    return shuffle_dirs_ss
                for dir_name in shuffle_dirs_ss:
                    dir_state = self.dir_states[dir_name]
                    if self.set_state(dir_state, State.STATE_SHUFFLING):
                        shuffle_dirs.append(dir_name)
                log.debug(f'remapping directories: {shuffle_dirs}')
                return shuffle_dirs

    def remove_instances(self, instance_ids):
        with self.lock:
            return self._remove_instances(instance_ids)

    def _remove_instances(self, instance_ids):
        log.debug(f'removing instances: {instance_ids}')
        shuffle_dirs = []
        for instance_id in instance_ids:
            if not instance_id in self.instance_to_dir_map:
                continue
            if not self.instance_to_dir_map[instance_id]:
                self.instance_to_dir_map.pop(instance_id)
                continue
            self.dead_instances.append(instance_id)
            dir_names = self.instance_to_dir_map[instance_id]
            log.debug(f'force shuffling instance_id {instance_id}, directories {dir_names}')
            for dir_name in dir_names:
                dir_state = self.dir_states[dir_name]
                if self.is_state_scheduled(dir_state, State.STATE_DISASSOCIATING):
                    log.debug(f'dir_name {dir_name} is disassociating, ignoring...')
                    continue
                log.debug(f'shuffling dir_name {dir_name}')
                if self.set_state(dir_state, State.STATE_SHUFFLING, True):
                    shuffle_dirs.append(dir_name)
        log.debug(f'shuffling {shuffle_dirs}')
        return shuffle_dirs

    def dir_status(self, dir_name):
        with self.lock:
            dir_state = self.dir_states.get(dir_name, None)
            if not dir_state:
                raise MirrorException(-errno.ENOENT, f'{dir_name} is not tracked')
            res = {}
            if dir_state.stalled:
                res['state'] = 'stalled'
                res['reason'] = 'no mirror daemons running'
            elif dir_state.state == State.STATE_ASSOCIATING:
                res['state'] = 'mapping'
            else:
                state = None
                dstate = dir_state.state
                if dstate == State.STATE_ASSOCIATING:
                    state = 'mapping'
                elif dstate == State.STATE_DISASSOCIATING:
                    state = 'unmapping'
                elif dstate == State.STATE_SHUFFLING:
                    state = 'shuffling'
                elif dstate == State.STATE_ASSOCIATED:
                    state = 'mapped'
                elif dstate == State.STATE_INITIALIZING:
                    state = 'resolving'
                res['state'] = state
                res['instance_id'] = dir_state.instance_id
                res['last_shuffled'] = dir_state.mapped_time
            return res

    def instance_summary(self):
        with self.lock:
            res = {}
            res['mapping'] = {}
            for instance_id, dir_names in self.instance_to_dir_map.items():
                res['mapping'][instance_id] = f'{len(dir_names)} directories'
            return res
