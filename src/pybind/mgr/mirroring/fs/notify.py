...
class Notifier():
    ...
    def notify_cbk(self, dir_name, callback):
        def cbk(_, r, acks, timeouts):
            log.debug(f'Notifier.notify_cbk: ret {r} acks: {acks} timeouts: {timeouts}')
            callback(dir_name, r)
        return cbk

    def notify(self, dir_name, message, callback):
        try:
            instance_id = message[0]
            message = message[1]
            log.debug(f'Notifier.notify: {instance_id} {message} for {dir_name}')
            self.ioctx.aio_notify(
                Notifier.instance_object(
                    instance_id), self.notify_cbk(dir_name, callback), msg=message)
