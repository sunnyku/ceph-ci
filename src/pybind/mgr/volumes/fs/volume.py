    ...
    def resize_subvolume(self, fs_handle, **kwargs):
        ...
        try:
            with SubVolume(self.mgr, fs_handle) as sv:
                ...
                nsize, usedbytes = sv.resize_subvolume(subvolpath, newsize)
                ret = 0, json.dumps([{'UsedInBytes': usedbytes},{'QuotaInBytes': nsize},
                                     {'PercentageUsed': '{0:.2f}'.format((float(usedbytes) / nsize) * 100.0)}], indent=2), ""
        except VolumeException as ve:
            ret = self.volume_exception_to_retval(ve)
        return ret
