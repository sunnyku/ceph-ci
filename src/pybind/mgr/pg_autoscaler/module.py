...
#                    p['pg_num_ideal'],
    ...
    def get_subtree_resource_status(self, osdmap, crush):
        ...
        # FIXME: avoid dumping out the whole PG map to python objects, use df
        # instead
    ...
    def _get_pool_status(
        ...
        for pool_name, p in pools.iteritems():
            ...
            adjust = final_pg_target > p['pg_num_target'] * threshold or final_pg_target <= p['pg_num_target'] / threshold
            ...
            if p['pg_autoscale_mode'] == 'on':
                ...
                if r[0] != 0:
                    ...
                    self.log.error("pg_num adjustment on {0} to {1} failed: {2}"
                                   .format(adj_intent.pool_name,
                                           adj_intent.new_pg_num_target, r))
