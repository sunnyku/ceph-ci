            ...
            for d in daemons:
                ...
                if code or json.loads(''.join(out)).get('image_id') != target_id:
                    ...
                    if r.get('image_id') != target_id:
                        self.clog_info('Upgrade: image %s pull on %s got new image %s (not %s), restarting' % (target_name, d.hostname, r['image_id'], target_id))
        ...
        for osd_id, osds in osds_elems.items():
            for osd in osds:
                ...
                if osd_uuid_map[osd_id] != osd['tags']['ceph.osd_fsid']:
                    ...
                    continue
                self.clog_info('Creating osd.%s on %s' % (osd_id, host))
                self._create_daemon(
                    'osd', osd_id, host,
