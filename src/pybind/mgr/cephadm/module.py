...
class CephadmOrchestrator(orchestrator.Orchestrator, MgrModule):
    ...
    def _refresh_configs(self):
        ...
        current_url = self.remote('dashboard', 'get_option', 'GRAFANA_API_URL')
        current_ssh_verify = self.remote('dashboard', 'get_option',
                                         'GRAFANA_API_SSL_VERIFY')
        self.log.debug('GRAFANA url %s verify %s, grafanas %s' % (
            current_url, current_ssh_verify, grafanas))
        if grafanas:
            ...
            # NOTE: we don't disable the dashboard grafana config becuase
            # we don't know if the user has set the stack up themselves.
            pass
