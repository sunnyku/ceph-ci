...
class HostCache():
    ...
    def save_host(self, host: str) -> None:
        ...
        if host in self.last_daemon_update:
            j['last_daemon_update'] = self.last_daemon_update[host].strftime(DATEFMT)
        if host in self.last_device_update:
            j['last_device_update'] = self.last_device_update[host].strftime(DATEFMT)
    ...
    def schedule_daemon_action(self, host: str, daemon_name: str, action: str):
        ...
        if existing_action and priorities[existing_action] > priorities[action]:
            logger.debug(f'skipping {action}ing {daemon_name}, cause {existing_action} already scheduled.')
            return
