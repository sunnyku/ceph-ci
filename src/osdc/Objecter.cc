...
void Objecter::try_resend() {
  map<ceph_tid_t, Op*> need_resend;
  map<ceph_tid_t, CommandOp*> need_resend_command;
  list<LingerOp*> need_resend_linger;

  if (!cct->_conf->objecter_pause_on_mon_lost) {
    return;
  }
  ldout(cct, 10) << "try_resend_requests" << dendl;
  ...
  return _try_resend(need_resend,
                         need_resend_linger,
                         need_resend_command,
                         sul);
}
...
void Objecter::_op_submit(Op *op, shunique_lock& sul, ceph_tid_t *ptid)
{
  ...
  if (!monc->is_connected() &&
      cct->_conf->objecter_pause_on_mon_lost) {
