...
//#define CACHE_BLOB_BL  // not sure if this is a win yet or not... :/
  ...
  */
  ...
  struct LruOnodeCacheShard : public OnodeCacheShard {
    ...
    LruOnodeCacheShard(CephContext *cct) : OnodeCacheShard(cct) {}
  ...
  /// An LRU buffer Cache Shard
  struct LruBufferCacheShard : public BufferCacheShard {
    typedef boost::intrusive::list<
      Buffer,
      boost::intrusive::member_hook<
        Buffer,
        boost::intrusive::list_member_hook<>,
        &Buffer::lru_item> > list_t;
    list_t lru;

    LruBufferCacheShard(CephContext *cct) : BufferCacheShard(cct) {}

    void _add(Buffer *b, int level, Buffer *near) override {
      if (near) {
    ...
    void _move(BufferCacheShard *src, Buffer *b) override {
      src->_rm(b);
      _add(b, 0, nullptr);
    }
    void _adjust_size(Buffer *b, int64_t delta) override {
      ceph_assert((int64_t)buffer_bytes + delta >= 0);
      buffer_bytes += delta;
    }
    void _touch(Buffer *b) override {
  ...
  /// A TwoQ buffer Cache Shard
  struct TwoQBufferCacheShard : public BufferCacheShard {
    typedef boost::intrusive::list<
      Buffer,
      boost::intrusive::member_hook<
        Buffer,
  ...
  public:
    TwoQBufferCacheShard(CephContext *cct) : BufferCacheShard(cct) {}
