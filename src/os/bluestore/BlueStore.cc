  ...
  uint32_t op_flags)
{
  ...
  {
    RWLock::RLocker l(c->lock);
    coarse_mono_time start1 = coarse_mono_clock::now();
    OnodeRef o = c->get_onode(oid, false);
    logger->tinc(l_bluestore_read_onode_meta_lat, coarse_mono_clock::now() - start1);
    if (!o || !o->exists) {
      r = -ENOENT;
      goto out;
    }
 ...
 out:
  ...
  dout(10) << __func__ << " " << cid << " " << oid
	   << " 0x" << std::hex << offset << "~" << length << std::dec
	   << " = " << r << dendl;
  logger->tinc(l_bluestore_read_lat, coarse_mono_clock::now() - start);
  return r;
}
  ...
  uint32_t op_flags)
{
  ...
  logger->tinc(l_bluestore_read_onode_meta_lat, coarse_mono_clock::now() - start);
  _dump_onode(o);
  ...
  start = coarse_mono_clock::now(); // for the sake of simplicity
  ...
  logger->tinc(l_bluestore_read_wait_aio_lat, coarse_mono_clock::now() - start);
			    ...
			    uint64_t logical_offset) const
{
  ...
  logger->tinc(l_bluestore_csum_lat, coarse_mono_clock::now() - start);
  return r;
}
...
int BlueStore::_decompress(bufferlist& source, bufferlist* result)
{
  ...
  logger->tinc(l_bluestore_decompress_lat, coarse_mono_clock::now() - start);
  return r;
}
...
void BlueStore::_kv_sync_thread()
{
  ...
  while (true) {
	...
	deferred_done.clear();
      ...
      coarse_mono_time after_flush = coarse_mono_clock::now();
	...
	dout(10) << __func__ << " blobid_max now " << blobid_max << dendl;
      ...
      {
        coarse_mono_time finish = coarse_mono_clock::now();
  ...
  ThreadPool::TPHandle *handle)
{
  ...
  coarse_mono_time tstart = coarse_mono_clock::now();
  ...
  coarse_mono_time tend = coarse_mono_clock::now();
  ...
  logger->tinc(l_bluestore_submit_lat, coarse_mono_clock::now() - start);
