...
namespace {
  ...
  struct Owner : public ElectionOwner {
    ...
    epoch_t read_persisted_epoch() { return persisted_epoch; }
    ...
    void message_victory(const set<int>& members) {
      for (auto i : members) {
	if (i == rank) continue;
    ...
    bool is_current_member(int rank) { return quorum.count(rank) != 0; }
