...
auto gen_usage_log_info(std::string user, std::string payer, std::string bucket, int total_usage_entries)
{
  rgw_usage_log_info info;
  for (int i=0; i < total_usage_entries; i++){
    info.entries.emplace_back(rgw_usage_log_entry(user, payer, bucket));
...
TEST(cls_rgw, usage_basic)
{
  ...
  info = gen_usage_log_info(user, payer, bucket1, 100);
  cls_rgw_usage_log_add(op, info);
  ASSERT_EQ(0, ioctx.operate(oid, &op));

  info = gen_usage_log_info(user, payer, bucket2, 100);
  cls_rgw_usage_log_add(op, info);
  ASSERT_EQ(0, ioctx.operate(oid, &op));
  ret = cls_rgw_usage_log_read(ioctx, oid, user, bucket1, start_epoch, end_epoch,
                              max_entries, read_iter, usage2, &truncated);
  ASSERT_EQ(0, ret);
  ASSERT_EQ(100, usage2.size());

  // delete and read to assert that bucket option is valid for usage trim
  ASSERT_EQ(0, cls_rgw_usage_log_trim(ioctx, oid, user, bucket1, start_epoch, end_epoch));

  ret = cls_rgw_usage_log_read(ioctx, oid, user, bucket1, start_epoch, end_epoch,
                               max_entries, read_iter, usage2, &truncated);
  ASSERT_EQ(0, ret);
  ASSERT_EQ(0, usage2.size());
  ASSERT_EQ(0, cls_rgw_usage_log_trim(ioctx, oid, user, bucket2, start_epoch, end_epoch));
}
