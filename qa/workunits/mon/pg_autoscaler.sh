...
wait_for 60 "ceph osd pool get a pg_num | grep 128"
wait_for 60 "ceph osd pool get b pg_num | grep 32"

# small ratio change does not change pg_num
ceph osd pool set a target_size_ratio .7
ceph osd pool set b target_size_ratio .2
sleep 10
ceph osd pool get a pg_num | grep 128
ceph osd pool get b pg_num | grep 32
