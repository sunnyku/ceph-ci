...
daemon_asok_file()
{
    ...
    echo $(ceph-conf --name "client.${MIRROR_USER_ID_PREFIX}${instance}" 'admin socket')
}
...
daemon_pid_file()
{
    ...
    echo $(ceph-conf --name "client.${MIRROR_USER_ID_PREFIX}${instance}" 'pid file')
}
...
setup_mirroring()
{
    ...
    CEPH_ARGS='' ceph fs ls --format xml > ${filesystems}
    fs=$($XMLSTARLET sel -t -v "//filesystems/filesystem/name" < ${filesystems})

    CEPH_ARGS='' ceph mgr module enable mirroring
    CEPH_ARGS='' ceph fs snapshot mirror enable ${fs}
}
