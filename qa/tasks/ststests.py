...
def _config_user(ststests_conf, section, user):
    ...
    ststests_conf[section].setdefault('secret_key',six.ensure_str(base64.b64encode(os.urandom(40))))
    ststests_conf[section].setdefault('display_name', 'Mr. {user}'.format(user=user))
            ...
            if section=='iam':
                ctx.cluster.only(client).run(
                    args=[
                        ...
                        '--display-name', ststests_conf[section]['display_name'],
