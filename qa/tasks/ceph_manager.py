        ...
        if self.ceph_objectstore_tool:
            ...
            for pool in self.ceph_manager.list_pools():
                ...
                cmd = ("CEPH_ARGS='--filestore-merge-threshold 1 "
                       "--filestore-split-multiple 1' sudo -E {prefix}"
                       "--op apply-layout-settings --pool {pool} 2>&1"
                       ).format(id=osd, prefix=no_sudo_prefix, pool=pool)
                output = remote.sh(cmd, wait=True, check_status=False)
