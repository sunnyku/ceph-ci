...
class TestSnapSchedules(CephFSTestCase):
    ...
    def fs_snap_schedule_cmd(self, *args):
        args = list(args)
        args.append("fs={0}".format(self.volname))
        return self._fs_cmd("snap-schedule", *args)
    ...
    def verify_schedule(self, dir_path, schedule_spec):
        ...
        result = json.loads(self.fs_snap_schedule_cmd("list", dir_path))
    ...
    def test_non_existent_snap_schedule_list(self):
        try:
            self.fs_snap_schedule_cmd("list", TestSnapSchedules.TEST_DIRECTORY)
    ...
    def test_snap_schedule_list_post_schedule_remove(self):
        ...
        self.fs_snap_schedule_cmd("add", TestSnapSchedules.TEST_DIRECTORY, '1m')

        self.fs_snap_schedule_cmd("remove", TestSnapSchedules.TEST_DIRECTORY)
    ...
    def test_snap_schedule(self):
        ...
        print("expecting snap {0}/.snap/scheduled-{1} in ~{2}s...".format(TestSnapSchedules.TEST_DIRECTORY, snap_sfx, timo))
        ...
        def verify_added(snaps_added):
            print("snapshots added={}".format(snaps_added))
            ...
            if snapname.startswith('scheduled-') and snapname[10:] == snap_sfx:
                ...
                print("exec={}, now = {}, timo = {}".format(exec_time, now, timo))
    ...
    def test_multi_snap_schedule(self):
        ...
        exec_time = time.time()
        self.fs_snap_schedule_cmd("add", TestSnapSchedules.TEST_DIRECTORY, '1m')
        self.fs_snap_schedule_cmd("add", TestSnapSchedules.TEST_DIRECTORY, '2m')

        timo_1, snap_sfx_1 = self.calc_wait_time_and_snap_name(exec_time, '1m')
        print("expecting snap {0}/.snap/scheduled-{1} in ~{2}s...".format(TestSnapSchedules.TEST_DIRECTORY, snap_sfx_1, timo_1))
        timo_2, snap_sfx_2 = self.calc_wait_time_and_snap_name(exec_time, '2m')
        print("expecting snap {0}/.snap/scheduled-{1} in ~{2}s...".format(TestSnapSchedules.TEST_DIRECTORY, snap_sfx_2, timo_2))
        ...
        def verify_added_1(snaps_added):
            ...
            if snapname.startswith('scheduled-') and snapname[10:] == snap_sfx_1:
                now = time.time()
                print("exec={}, now = {}, timo = {}".format(exec_time, now, timo_1))
                # snap should not prematurely come into existance
        ...
        def verify_added_2(snaps_added):
            ...
            if snapname.startswith('scheduled-') and snapname[10:] == snap_sfx_2:
                now = time.time()
                print("exec={}, now = {}, timo = {}".format(exec_time, now, timo_2))
                # snap should not prematurely come into existance
    ...
    def test_snap_schedule_with_retention(self):
        ...
        # set a schedule on the dir
        exec_time = time.time()
        self.fs_snap_schedule_cmd("add", TestSnapSchedules.TEST_DIRECTORY, '1m', 'retention-policy=3m')

        timo_1, snap_sfx = self.calc_wait_time_and_snap_name(exec_time, '1m')
        print("expecting snap {0}/.snap/scheduled-{1} in ~{2}s...".format(TestSnapSchedules.TEST_DIRECTORY, snap_sfx, timo_1))
        timo_2, _ = self.calc_wait_time_and_snap_name(exec_time, '3m')
        print("expecting removal of snap {0}/.snap/scheduled-{1} in ~{2}s...".format(TestSnapSchedules.TEST_DIRECTORY, snap_sfx, timo_2))
        to_wait = timo_2 + 2 # some leeway to avoid false failures...

        # verify snapshot schedule
        self.verify_schedule(TestSnapSchedules.TEST_DIRECTORY, [['1m', '3m']])
        ...
        def verify_removed(snaps_removed):
            print("snapshots removed={}".format(snaps_removed))
