...
class TestVolumes(CephFSTestCase):
    ...
    def _enable_multi_fs(self):
        self._volume_cmd("flag", "set", "enable_multiple", "true", "--yes-i-really-mean-it")

    def _create_or_reuse_test_volume(self):
        result = json.loads(self._volume_cmd("volume", "ls"))
        if len(result) == 0:
            self.vol_created = True
            self.volname = TestVolumes.TEST_VOLUME_NAME
            self._volume_cmd("volume", "create", self.volname)
        else:
            self.volname = result[0]['name']

    def _delete_test_volume(self):
        self._volume_cmd("volume", "rm", self.volname)
    ...
    def test_subvolume_create_and_rm(self):
        ...
        self._volume_cmd("subvolume", "create", self.volname, subvolume)

        # make sure it exists
        subvolpath = self._volume_cmd("subvolume", "getpath", self.volname, subvolume)
        self.assertNotEqual(subvolpath, None)

        # remove subvolume
        self._volume_cmd("subvolume", "rm", self.volname, subvolume)
        # make sure its gone
        try:
            self._volume_cmd("subvolume", "getpath", self.volname, subvolume)
    ...
    def test_nonexistent_subvolume_rm(self):
        ...
        self._volume_cmd("subvolume", "rm", self.volname, subvolume, "--force")
    ...
    def test_nonexistent_subvolume_group_create(self):
        ...
        try:
            self._volume_cmd("subvolume", "create", self.volname, subvolume, "--group_name", group)
        except CommandFailedError as ce:
            if ce.exitstatus != errno.ENOENT:
                raise
    ...
    def test_subvolume_create_and_rm_in_group(self):
        ...
        # create group
        self._volume_cmd("subvolumegroup", "create", self.volname, group)

        # create subvolume in group
        self._volume_cmd("subvolume", "create", self.volname, subvolume, "--group_name", group)

        # remove subvolume
        self._volume_cmd("subvolume", "rm", self.volname, subvolume, group)

        # remove group
        self._volume_cmd("subvolumegroup", "rm", self.volname, group)
    ...
    def test_nonexistent_subvolme_group_rm(self):
        ...
        # force remove subvolume
        self._volume_cmd("subvolumegroup", "rm", self.volname, group, "--force")
    ...
    def test_subvolume_snapshot_create_and_rm(self):
        ...
        # snapshot subvolume
        self._volume_cmd("subvolume", "snapshot", "create", self.volname, subvolume, snapshot)

        # remove snapshot
        self._volume_cmd("subvolume", "snapshot", "rm", self.volname, subvolume, snapshot)
    ...
    def test_nonexistent_subvolume_snapshot_rm(self):
        ...
        self._volume_cmd("subvolume", "snapshot", "rm", self.volname, subvolume, snapshot, "--force")
    ...
    def test_subvolume_snapshot_in_group(self):
        ...
        self._volume_cmd("subvolume", "snapshot", "create", self.volname, subvolume, snapshot, group)

        # remove snapshot
        self._volume_cmd("subvolume", "snapshot", "rm", self.volname, subvolume, snapshot, group)
    ...
    def test_subvolume_group_snapshot_create_and_rm(self):
        ...
        # snapshot group
        self._volume_cmd("subvolumegroup", "snapshot", "create", self.volname, group, snapshot)

        # remove snapshot
        self._volume_cmd("subvolumegroup", "snapshot", "rm", self.volname, group, snapshot)
