...
class TestEphemeralPins(CephFSTestCase):
    ...
    def test_ephemeral_pin_distribution_after_child_create(self):
        """
        That ephemerally pinned subtrees are somewhat evenly distributed. The xattr 
        is set after child directories are created.
        """
    ...
    def test_ephemeral_pin_distribution_before_child_create(self):
        """
        That ephemerally pinned subtrees are somewhat evenly distributed. The xattr
        is set before child directories are created.
        """
    ...
    def test_ephemeral_random_dist(self):
        """
        That ephemeral random and distributed can co-exist with each other
        """
        ...
        for i in range(0, 100):
          tmp_dir = tmp_dir + str(i) + "/"
          self.mount_a.run_shell(["mkdir", "-p", "a/" + tmp_dir])
          self.mount_a.setfattr("a/" + temp_dir, "ceph.dir.pin.random", "1")
    ...
    def test_ephemeral_pin_grow_mds(self):
        ...
        subtrees_old = self.get_ephemerally_pinned_auth_subtrees(status, 0) + self.get_ephemerally_pinned_auth_subtrees(status, 1)
        self.fs.set_max_mds(3)
        self.fs.wait_for_daemons()
        ...
        status = self.fs.status()
        subtrees_new = self.get_ephemerally_pinned_auth_subtrees(status, 0) + self.get_ephemerally_pinned_auth_subtrees(status, 1) + self.get_ephemerally_pinned_auth_subtrees(status, 2) 
        count = 0
    ...
    def test_ephemeral_standby(self):
        ...
        original_subtrees = self.get_ephemerally_pinned_auth_subtrees(status, 1)
        ...
        self.fs.mds_stop(status.get_rank(self.fs.id, 1)['name'])

        self.fs.wait_for_state('up:active', rank=1, timeout=60)

        time.sleep(30)
        
        status = self.fs.status()
        new_subtrees = self.get_ephemerally_pinned_auth_subtrees(status, 1)
