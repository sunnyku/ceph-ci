...
yet been acknolwedged or archived by the administrator.  This may
...
warning and is visible via ``ceph crash ls`` but not ``ceph crash
ls-new``::
...
``mgr/crash/warn_recent_interval`` and defaults to two weeks.
